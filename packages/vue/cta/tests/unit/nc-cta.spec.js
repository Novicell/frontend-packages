...
import NcCta from '@/nc-cta.vue'
import '@novicell/styles-cta'

describe('nc-cta.vue', () => {
  it('renders an NcLink with an svg icon and slot text', () => {

    const wrapper = mount(NcCta, {
      propsData: {
        name: 'Google',
        url: 'https://google.dk'
      },
    ...
    const textContent = Array.from(link.element.childNodes).find(node => node.nodeName === '#text').textContent
    expect(textContent.includes('Google')).toBe(true)
