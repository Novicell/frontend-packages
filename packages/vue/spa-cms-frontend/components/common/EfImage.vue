...
export default class EfImage extends Vue {
  ...
  public render(h: CreateElement): VNode {
    ...
    if (
      this.image.focalPoint
      && this.image.focalPoint.top
      && this.image.focalPoint.left
    ) {
  ...
  get imageSizes() {
    ...
    return this.sizes
      .reduce((arr: string[], currentValue: ImageSize) => {
        const str = `${url}?width=${currentValue.imageWidth} ${
          currentValue.windowWidth ? `${currentValue.windowWidth}w` : ''
        }`;
        arr.push(str);
        return arr;
      }, [])
